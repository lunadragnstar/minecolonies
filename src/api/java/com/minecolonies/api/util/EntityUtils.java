...
public final class EntityUtils
{
    ...
    /**
     * {@link #isWorkerAtSiteWithMove(IEntityCitizen, int, int, int, int)}.
     *
     ...
     */
    public static boolean isWorkerAtSiteWithMove(@NotNull final IEntityCitizen worker, final int x, final int y, final int z)
    {
     ...
     * @return True if worker is at site, otherwise false.
     */
    public static boolean isWorkerAtSiteWithMove(@NotNull final IEntityCitizen worker, final int x, final int y, final int z, final int range)
    {
        if (!isWorkerAtSite(worker, x, y, z, TELEPORT_RANGE))
        {
            ...
            worker.setLocationAndAngles(
              ...
              worker.getRotationYaw(),
              worker.getRotationPith());
            return true;
        }
        //If too far away
        if (!isWorkerAtSite(worker, x, y, z, range))
        {
            ...
            if (worker.getNavigator().noPath() && !tryMoveLivingToXYZ((EntityLiving) worker, x, y, z))
            {
     ...
     */
    public static boolean setSpawnPoint(@Nullable BlockPos spawnPoint, @NotNull IEntityCitizen citizen)
    {
        if(spawnPoint == null)
        {
            return false;
     ...
     * @return True if worker is at site, otherwise false
     */
    public static boolean isWorkerAtSite(@NotNull final IEntityCitizen worker, final int x, final int y, final int z, final int range)
    {
